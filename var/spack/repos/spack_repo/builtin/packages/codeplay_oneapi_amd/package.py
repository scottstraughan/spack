...
class CodeplayOneapi:
    ...
    def get_target_driver_version(self):
        ...
        if "driver" in self.spec.variants:
            ...
            if target_driver_version == "system":
                if self.backend_name in self.spec:
                    ...
                    tty.msg(
                        f"User did not specific target driver and could not find a system version."
                    )
            else:
                tty.msg(
...
class CodeplayOneapiAmd(Package):
    ...
    variant(
        "driver", default="system", values=drivers, description="Change the ROCm driver version"
    )
